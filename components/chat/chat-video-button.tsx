...
export const ChatVideoButton = () => {
  ...
  const VideoIcon = isVideo ? VideoOff : Video; // if the video is already active, show the video off icon, otherwise show the video icon
    ...
    const url = qs.stringifyUrl(
      {
        url: pathname || "",
        query: {
          // if the video is already active, remove it, otherwise add it
  ...
  return (
    <ActionTooltip side="bottom" label={tooltipLabel}>
      <button onClick={onClick} className="hover:opacity-75 transition mr-4">
        <VideoIcon className="size-6 text-zinc-500 dark:text-zinc-400" />
      </button>
    </ActionTooltip>
  );
};
